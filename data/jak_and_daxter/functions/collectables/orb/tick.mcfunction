...
execute as @e[tag=jad.orb] at @s if entity @a[distance=0..1.5] if function jak_and_daxter:collectables/orb/check on passengers if entity @s[tag=jad.data] if data entity @s {data:{collected:0}} on vehicle run function jak_and_daxter:collectables/orb/collect
...
execute as @e[tag=jad.orb] on passengers if entity @s[tag=jad.data] if score @s jad.math_handling <= reset jad.value run function jak_and_daxter:collectables/orb/reset
