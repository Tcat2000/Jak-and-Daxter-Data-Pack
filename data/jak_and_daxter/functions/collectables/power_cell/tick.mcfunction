...
execute as @e[tag=jad.power_cell] at @s if entity @a[distance=0..1.5] if function jak_and_daxter:collectables/power_cell/check on passengers if entity @s[tag=jad.data] if data entity @s {data:{collected:0}} on vehicle run function jak_and_daxter:collectables/power_cell/collect
...
execute as @e[tag=jad.power_cell] on passengers if entity @s[tag=jad.data] if score @s jad.math_handling <= reset jad.value run function jak_and_daxter:collectables/power_cell/reset
